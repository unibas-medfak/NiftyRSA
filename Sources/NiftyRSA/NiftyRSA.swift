...
public enum NiftyRSA {
    ...
    static func base64String(pemEncoded pemString: String) throws -> String {
        ...
        guard lines.count != 0 else {
            throw NiftyRSAError.pemDoesNotContainKey
        }
    ...
    static func data(forKeyReference reference: SecKey) throws -> Data {
        ...
        guard let unwrappedData = data as Data? else {
            throw NiftyRSAError.keyRepresentationFailed(error: error?.takeRetainedValue())
        }
    ...
    public static func generateRSAKeyPair(sizeInBits size: Int) throws -> (privateKey: NiftyRSAPrivateKey, publicKey: NiftyRSAPublicKey) {
        return try generateRSAKeyPair(sizeInBits: size, applyUnitTestWorkaround: false)
    }

    static func generateRSAKeyPair(sizeInBits size: Int, applyUnitTestWorkaround: Bool = false) throws -> (privateKey: NiftyRSAPrivateKey, publicKey: NiftyRSAPublicKey) {
        ...
        guard let privKey = SecKeyCreateRandomKey(attributes as CFDictionary, &error),
            ...
            throw NiftyRSAError.keyGenerationFailed(error: error?.takeRetainedValue())
        }
        let privateKey = try NiftyRSAPrivateKey(reference: privKey)
        let publicKey = try NiftyRSAPublicKey(reference: pubKey)
    ...
    static func addKey(_ keyData: Data, isPublic: Bool, tag: String) throws -> SecKey {
        ...
        guard tag.data(using: .utf8) != nil else {
            throw NiftyRSAError.tagEncodingFailed
        }
        ...
        guard let key = SecKeyCreateWithData(keyData as CFData, keyDict as CFDictionary, &error) else {
            throw NiftyRSAError.keyCreateFailed(error: error?.takeRetainedValue())
        }
        return key
    }
    ...
    static func stripKeyHeader(keyData: Data) throws -> Data {
        ...
        throw NiftyRSAError.invalidAsn1Structure
    }
