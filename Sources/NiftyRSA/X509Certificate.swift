...
extension NSInteger {
    fileprivate func encodedOctets() -> [CUnsignedChar] {
        // Short form
        if self < 128 {
    ...
    fileprivate init?(octetBytes: [CUnsignedChar], startIdx: inout NSInteger) {
...
extension Data {
    ...
    public func prependx509Header() -> Data {
    ...
    public func hasX509Header() throws -> Bool {
    ...
    public func isAnHeaderlessKey() throws -> Bool {
        let node: Asn1Parser.Node
        do {
            node = try Asn1Parser.parse(data: self)
        }
